    ...
    <div class="fixed inset-0 bg-black/80 backdrop-blur-md z-[200] flex items-center justify-center p-2 md:p-4" onClick={onClose}>
      <div class="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg h-[90vh] flex flex-col overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-700 shrink-0">
          ...
          <button onClick={onClose} class="text-slate-400 hover:text-white"><i class="fas fa-times text-lg"></i></button>
        </div>
        {mode === "chat" && (<>
          ...
          <div class="p-3 border-t border-slate-700 flex gap-2"><input ref={inputRef} type="text" value={input} onInput={e => setInput((e.target as HTMLInputElement).value)} onKeyDown={handleKeyDown} class="flex-1 bg-slate-800 text-xs text-white px-4 py-2.5 rounded-xl border border-slate-700 focus:outline-none focus:border-amber-500" placeholder={t("ai.placeholder_admin")} /><button onClick={() => handleSend()} disabled={sending} class="bg-amber-600 hover:bg-amber-500 text-white px-4 py-2.5 rounded-xl disabled:opacity-50"><i class="fas fa-paper-plane"></i></button></div>
        </>)}
        {mode === "parse" && <div class="flex-1 overflow-y-auto p-4 space-y-3">
          <div class="bg-slate-800/50 border border-slate-700 rounded-xl p-3"><label class="block text-[10px] font-bold text-slate-400 mb-1">Upload CV/Excel/PDF</label><input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,image/*" onChange={e => { const f = (e.target as HTMLInputElement).files?.[0]; if (f) setParseFile(f); }} class="w-full text-[10px] text-slate-400 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-amber-600 file:text-white file:text-[10px] file:font-bold" />{parseFile && <p class="text-[10px] text-emerald-400 mt-1"><i class="fas fa-check mr-0.5"></i>{parseFile.name}</p>}</div>
          <div class="grid grid-cols-2 gap-2"><input type="tel" value={parseWa} onInput={e => setParseWa((e.target as HTMLInputElement).value)} placeholder="WA kandidat" class="bg-slate-800 text-xs text-white px-3 py-2 rounded-lg border border-slate-700" /><input type="text" value={parseBidang} onInput={e => setParseBidang((e.target as HTMLInputElement).value)} placeholder="Bidang SSW" class="bg-slate-800 text-xs text-white px-3 py-2 rounded-lg border border-slate-700" /></div>
          <div class="flex gap-2"><button onClick={handleParse} class="flex-1 py-2.5 bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold rounded-lg"><i class="fas fa-bolt mr-1"></i>Parse</button><button onClick={handleGenModel} class="flex-1 py-2.5 bg-sky-600 hover:bg-sky-500 text-white text-xs font-bold rounded-lg"><i class="fas fa-clipboard-list mr-1"></i>Model</button></div>
          <div class="flex gap-2"><button onClick={handleResults} class="flex-1 py-2 bg-slate-700 hover:bg-slate-600 text-white text-[11px] font-bold rounded-lg"><i class="fas fa-file-alt mr-1 text-sky-400"></i>Hasil</button><button onClick={handleUpdateBio} class="flex-1 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-bold rounded-lg"><i class="fas fa-database mr-1"></i>Update Bio</button></div>
        ...
        {mode === "results" && <div class="flex-1 overflow-y-auto p-4">
          {lastHasil ? <div class="bg-slate-800/50 border border-slate-700 rounded-xl p-4 space-y-2"><h4 class="text-sm font-bold text-amber-400">Hasil Wawancara</h4><p class="text-xs text-slate-300">Kandidat: <span class="text-white font-bold">{(lastHasil as any).nama || (lastHasil as any).wa}</span></p><p class="text-xs text-slate-300">Skor: <span class="text-emerald-400 font-bold">{(lastHasil as any).hasil?.score ?? "-"}/10</span></p>{(lastHasil as any).hasil?.rekomendasi && <p class="text-xs text-slate-300">Rekomendasi: {(lastHasil as any).hasil.rekomendasi}</p>}<button onClick={handleUpdateBio} class="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-lg mt-2"><i class="fas fa-database mr-1"></i>Update Biodata</button></div> : <p class="text-xs text-slate-500 text-center py-8">Belum ada hasil.</p>}
        </div>}
      </div>
    </div>
  );
