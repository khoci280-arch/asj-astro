...
export default function TabWA() {
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      const d = await apiClient('saveWaTemplate', body);
      if (d.success) { alert(editingId ? 'Template updated!' : 'Template saved!'); mutate(); setEditingId(''); setNama(''); setIsi(''); }
      else alert('Failed: ' + (d.error || 'Unknown'));
    } catch (e) { alert('Error: ' + e); }
  }

  function handleEdit(t: any) { setEditingId(t.id); setNama(t.nama); setIsi(t.isi); }
  function handleCancel() { setEditingId(''); setNama(''); setIsi(''); }
  function handleDelete(id: string) {
    if (!confirm('Hapus template ini?')) return;
    apiClient('deleteWaTemplate', { id }).then(d => { if (d.success) mutate(); else alert('Gagal: ' + d.error); }).catch(e => alert('Error: ' + e));
  }
  ...
  if (isLoading) return <div class="text-center py-8"><i class="fas fa-spinner fa-spin text-2xl text-emerald-400"></i><p class="text-slate-500 mt-2 text-sm">Memuat template...</p></div>;
