...
export default function TabTambah() {
  ...
  if (loading) return <div class='text-center py-8'><Icon spin name="spinner" class="text-2xl text-red-400" /><p class='text-slate-500 mt-2 text-sm'>Memuat form...</p></div>;
  ...
  return (<div>
    <h2 class='text-red-400 font-bold mb-6 border-b border-red-900/50 pb-3 text-lg'><Icon name="plus-circle" class="mr-2" /> Form Input Loker Baru</h2>
    <form onSubmit={handleSubmit} class='grid grid-cols-1 md:grid-cols-2 gap-6'>
      <div class='space-y-5'>
        <div class='p-4 bg-red-900/10 border border-red-900/30 rounded-xl grid grid-cols-2 gap-4'>
          <div class='col-span-2'><label class='block text-xs font-bold text-red-400 mb-1 uppercase tracking-widest'><Icon name="lock" class="mr-1" /> Info DB Internal</label></div>
      ...
      <div class='space-y-5'>
        ...
        <div><label class={lc}>SYARAT KANDIDAT (Pilih Checkbox)</label>
          <div class='relative mb-2'><Icon name="search" class="absolute left-3 top-3 text-slate-500 text-sm" /><input type='text' placeholder='Cari syarat...' class='w-full pl-8 p-2.5 rounded-lg bg-black/40 border border-slate-700 text-sm text-white outline-none focus:border-red-500 transition' /></div>
        ...
        <div><label class={lc}><Icon name="file-upload" class="mr-1" /> SYARAT UPLOAD DOKUMEN</label>
      ...
      <div class='md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6'>
        <div><label class='block text-sm font-bold text-emerald-400 mb-1.5 uppercase'><Icon name="wallet" class="mr-1" /> TOTAL BIAYA JOB (Cth: 25 JT)</label><input type='text' value={totalBiaya} onInput={(e)=>setTotalBiaya((e.target as HTMLInputElement).value)} placeholder='Contoh: 25 JT' class={ic} /></div>
        <div><label class='block text-sm font-bold text-emerald-400 mb-1.5 uppercase'><Icon name="list-check" class="mr-1" /> RINCIAN BIAYA & TAHAPAN</label><button type='button' class='w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-sm font-black uppercase shadow-lg transition'><Icon name="edit" class="mr-1" /> Buka Editor Rincian</button><div class='text-xs font-bold text-emerald-300 mt-1.5 min-h-[16px]'>Klik untuk isi rincian biaya</div></div>
      </div>
      <div class='md:col-span-2 mt-4'>
        <button type='submit' disabled={submitting} class='w-full py-4 rounded-xl bg-red-600 hover:bg-red-500 font-black tracking-widest text-white shadow-[0_0_15px_rgba(220,38,38,0.5)] transition text-base disabled:opacity-50'>
          <Icon name="cloud-upload-alt" spin={submitting} class="mr-2" /> {submitting?'MENGUNGGAH...':'UNGGAH DATA LOKER BARU'}
        </button>
      </div>
    </form>
  </div>);
